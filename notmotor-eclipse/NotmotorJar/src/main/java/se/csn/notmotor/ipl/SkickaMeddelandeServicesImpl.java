...
/**
 * Implementering av tjansten for att skicka meddelanden.
 */
public class SkickaMeddelandeServicesImpl extends MeddelandeServicesImplBase implements SkickaMeddelandeServices {
            ...
            try {
                ...
                for (String namn : paramSource.getStringParam("KANALER_MED_BEGRANSNINGAR", "").split("[, ]+")) {
                    ...
                    if (soverTimestamp > 0) {
                        ...
                        if (!kanal.isSovande()) {
                            // Om vi har sovit men nyligen vaknat så nollställer vi sov-tidsstämpeln i
                            // parameterkällan.
                ...
                for (Kanal kanal : kanalerMedBegransningar) {
                    if (!kanal.isSovande() && kanal.getBatchStorlek() > 0) {
                        ...
                        paramSource.setIntParam(kanal.getBatchKvarKey(),
                            kanal.getBatchKvar() - kanal.getAntalMarkerade());
                ...
                if (log.isInfoEnabled() && foundMessages) {
                    ...
                    log.info("Tid för att markera och hämta meddelanden: " + tid2 + " ms (varav " + tid
                        + " ms för att markera).");
                }
                updateWatchdogFlag();
                conn.commit();
                ...
                for (Iterator it = meddelanden.iterator();it.hasNext();) {
                    ...
                    if (sandresultat.size() == 0) {
                        ...
                        handelse = new MeddelandeHandelse(MeddelandeHandelse.SKICKAT_SERVER, MeddelandeHandelse.OK,
                            null);
                        for (Iterator resit = sandresultat.iterator();resit.hasNext();) {
                            ...
                            for (int mottloop = 0;mottloop < mott.length;mottloop++) {
                                mott[mottloop].setStatus(Integer.valueOf(sr.getHandelsetyp()));
                            ...
                            if (sr.getHandelsetyp() != MeddelandeHandelse.SKICKAT_SERVER) {
                                handelse.setHandelsetyp(Integer.valueOf(sr.getHandelsetyp()));
                                handelse.setFelkod(Integer.valueOf(sr.getKod()));
                                handelse.setFeltext(sr.getText());
                            }
                        }
                    }

                    handelse.setInstans(Integer.valueOf(instansnummer));
    ...
    /**
     * Metod som kontrollerar om antal sandningar har overskridit en viss grans,
     * MAXSANDFORSOK. Satter status till TEKNISKT_FEL om gransen overskridits. Max
     * antal sandningsforsok styrs av parametern MAXSANDFORSOK.
     * 
    ...
    boolean forMangaSandningar(Meddelande meddelande) {
        ...
        if (meddelande.getHandelser().length > maxSandForsok) {
            MeddelandeHandelse h = new MeddelandeHandelse(MeddelandeHandelse.TEKNISKT_FEL, MeddelandeHandelse.OKANT_FEL,
                "Satte status till TEKNISKT_FEL pga för många sändningsförsök");
            h.setInstans(Integer.valueOf(instansnummer));
            meddelande.addHandelse(h);
            daohandelse.createHandelse(h, meddelande.getId().longValue());
            log.error("Sätter status för meddelande " + meddelande.getId().longValue()
                + " till TEKNISKT_FEL pga för många händelser");
            qp.executeThrowException("UPDATE MEDDELANDE SET STATUS = " + MeddelandeHandelse.TEKNISKT_FEL
                + " WHERE ID = " + meddelande.getId().longValue());
            try {
    ...
    /**
     * Metod som kollar om meddelandet nyligen sants med tekniskt fel.
     * Tid till senaste handelse styrs av parametern MINTIDTILLSENASTEFEL;
     * 
     * @param meddelande DTOMeddelande
     * @return true om meddelandet "nyligen" sändes med resultatet TEKNISKT_FEL
     *         och då väntar vi med att skicka det till nästa tick,
     *         annars false vilket innebär att det kommer att försöka sändas igen på
     *         en gång.
     */
    ...
    /**
     * Gor callbackanrop om sadana efterfragats.
     * 
     * @param meddelande DTOMeddelande
