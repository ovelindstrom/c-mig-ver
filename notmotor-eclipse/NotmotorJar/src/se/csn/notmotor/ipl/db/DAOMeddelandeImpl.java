...
public class DAOMeddelandeImpl extends DAOImplBase implements RowToObjectMapper, DAOMeddelande {
     ...
     * 
     * En kanal med begränsningar kommer alltid att bearbetas efter kanaler utan begränsningar,
     * dvs ha en lägre prioritet. Om det finns flera begränsade kanaler kommer de att
     ...
     * MEDDELANDE - HANDELSE.  
     * 
     * @param instans Den instans som markeras som "ägare" till meddelandena
     * @param antalMeddelanden Det maximala antal meddelanden som markeras
     * @param kanalerMedBegransningar Lista med nedprioriterade inkanaler med särskilda begränsningar
     */
	...
	public void markeraMeddelandenForInstans(int instans, int antalMeddelanden, List<Kanal> kanalerMedBegransningar) {
        ...
        for (Kanal kanal : kanalerMedBegransningar) {
        	...
        	if (kanal.isOppen()) {
        		...
        		int antal = Math.min(Math.min(kvarAttSkickaIBatch, batchstorlek), Math.min(kvarAttSkickaDennaTimme, maxAntal));
        		/*
        		if (log.isDebugEnabled()) {
	...
	public Object newRow(ResultSet rs) throws SQLException {
		...
		m.setCallbackMask(new Integer(rs.getInt("CALLBACKMASK")));
		
		m.setCsnnummer(new Integer(rs.getInt("CSNNUMMER")));
