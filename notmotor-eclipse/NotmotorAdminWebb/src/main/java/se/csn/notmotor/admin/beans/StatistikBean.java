...
public class StatistikBean {
    ...
    public static class DatumradRowMapper implements RowToObjectMapper {
        public Object newRow(ResultSet rs) throws SQLException {
            return new Datumrad(rs.getString("DATUM"), rs.getInt("INSTANS"), rs.getInt("STATUS"), null,
                rs.getInt("ANTAL"));
        }
    }
    ...
    /**
     * Hamtar all tillganglig statistik om mottagna, sanda och borttagna
     * meddelanden. All statistik lagras i bonan och finns tillgangliga via
     * get-metoderna.
     */
     ...
     * 
     * @param statistik      befintlig statistik, där varje kanal mappas mot ett
     *                       Statistik-objekt.
     *                       Kanalen null motsvarar den totala summan av
     *                       statistiken.
     *                       Denna Map MÅSTE tillåta null-keys!!!
     * @param statistikrader en lista med nya statistikrader. Varje
     *                       Statistikrad-objekt representerar en kanal.
    ...
    /**
     * Returnerar aktuellt varde for given kanal utifran lista med
     * statistikradobjekt.
     * Om kanal ar null sa returneras summan av alla statistikradobjekt.
     * 
     * @param statistikrader lista med statistikradobjekt
     * @param kanal          den kanal som söks. Null ger totala summan.
     * @return värdet (eller summan) från givna statistikradobjekt, beroende på
     *         kanal
     */
    ...
    private List<Statistikrad> getSantSedan(Date tid) {
        ...
        return qp.processQuery("SELECT KANAL, COUNT(*) as ANTAL FROM MEDDELANDE "
            + "WHERE SANTTIDPUNKT >= " + DAOImplBase.quoteValue(tid) + " GROUP BY KANAL",
            new StatistikradRowMapper());
    }

    /**
     * Hamtar alla sanda meddelanden grupperade per kanal for angivet datum, dsv
     * under aktuell dag. Skulle dag vara null kommer dagens datum att anvandas.
     * 
    ...
    private List<Statistikrad> getMottagnaSedan(Date tid) {
        ...
        return qp.processQuery("SELECT KANAL, COUNT(*) as ANTAL FROM MEDDELANDE "
            + "WHERE SKAPADTIDPUNKT >= " + DAOImplBase.quoteValue(tid) + " GROUP BY KANAL",
            new StatistikradRowMapper());
    }

    /**
     * Hamtar alla mottagna meddelanden grupperade per kanal for angivet datum, dsv
     * under aktuell dag.
     * Skulle dag vara null kommer dagens datum att anvandas.
     * 
     * @param dag den dag som statistik ska hämas för
    ...
    /**
     * Returnerar foregaende dag i kalendern baserat pa attributet valdDag.
     * 
    ...
    /**
     * Returnerar nasta dag i kalendern baserat pa attributet valdDag.
     * 
    ...
    /**
     * Returnerar inskickat datum som en formaterad datumstrang pa formen
     * yyyy-MM-dd. Skulle
     * datum vara null kommer dagens datum att anvandas.
     * 
     * @param datum det datum som ska formteras. Felaktigt datum eller null ger
     *              dagens datum.
     * @return datumsträng formaterat enligt yyyy-MM-dd.
     */
    ...
    /**
     * Returnerar inskickad datumstrang som en Date. Skulle datumstangen vara
     * felaktigt eller
     * null kommer dagens datum att returneras.
     * 
     * @param datum på formen yyyy-MM-dd. Felaktigt format eller null ger dagens
     *              datum.
